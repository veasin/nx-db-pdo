...
//基本类型
test::case('int', (string)new part(1, type::VALUE, $sql))->toBe('1');
test::case('string', (string)new part('1', type::VALUE, $sql))->toBe('"1"');
test::case('string', (string)new part('abc', type::VALUE, $sql))->toBe('"abc"');
test::case('string as', (string)new part('abc', type::VALUE, $sql)->as('name'))->toBe('"abc" `name`');
...
//field
test::case('field string', (string)new part('id', type::FIELD, $sql))->toBe('`id`');
test::case('field int', (string)new part('123', type::FIELD, $sql))->toBe('`123`');
test::case('field from', (string)$sql['id'])->toBe('`id`');

//function
$part = new part('sum', type::FUNCTION, $sql);
...
$sql = new sql('test');
$part = new part('CONCAT', type::FUNCTION, $sql);
...
test::case('between not', (string)sql::between(1,2,3)->negate())->toBe('1 NOT BETWEEN 2 AND 3');
test::case('in', (string)sql::in(1,1,2,3))->toBe('1 IN (1,2,3)');
test::case('not in', (string)sql::not_in(1,1,2,3))->toBe('1 NOT IN (1,2,3)');
test::case('if', (string)sql::IF(true, 'true', 'false'))->toBe('IF(TRUE, "true", "false")');
test::case('if', (string)sql::IF(true, 'true', 'false'))->toBe('IF(TRUE, "true", "false")');
test::case('if', (string)sql::IF(null, 'true', 'false'))->toBe('IF(NULL, "true", "false")');
test::case('if', (string)sql::IF('*', '\*', 'no'))->toBe('IF(*, "*", "no")');
...
test::case('+', (string)sql::add(1,2))->toBe('1 + 2');
test::case('not', (string)sql::like(1,2)->negate())->toBe('1 NOT LIKE 2');
test::case('>', (string)sql::gt(1,2))->toBe('1 > 2');
test::case('>', (string)sql::gt(1,2)->negate())->toBe('NOT (1 > 2)');
test::case('>', (string)sql::not_gt(1,2))->toBe('NOT (1 > 2)');
