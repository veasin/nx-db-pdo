 ...
 * @method static part CONCAT(...$strings) 返回结果为连接参数产生的字符串。如有任何一个参数为NULL ，则返回值为 NULL。或许有一个或多个参数。 如果所有参数均为非二进制字符串，则结果为非二进制字符串。 如果自变量中含有任一二进制字符串，则结果为一个二进制字符串。一个数字参数被转化为与之相等的二进制字符串格式；若要避免这种情况，可使用显式类型 cast, 例如：
 ...
 * @method static part EXPORT_SET($on,$off,$separator,$number_of_bits) 返回值为一个字符串，其中对于bits值中的每个位组，可以得到一个 on 字符串，而对于每个清零比特位，可以得到一个off 字符串。bits 中的比特值按照从右到左的顺序接受检验
 *         (由低位比特到高位比特)。字符串被分隔字符串分开(默认为逗号‘,’)，按照从左到右的顺序被添加到结果中
 * @method static part FIELD(...$strings) 返回值为str1, str2, str3,……列表中的str 指数。在找不到str 的情况下，返回值为 0
 * @method static part FIND_IN_SET($strings) 假如字符串str 在由N 子链组成的字符串列表strlist 中， 则返回值的范围在 1 到 N 之间 。一个字符串列表就是一个由一些被‘,’符号分开的自链组成的字符串。如果第一个参数是一个常数字符串，而第二个是type SET列，则   FIND_IN_SET() 函数被优化，使用比特计算。如果str不在strlist
 *         或strlist 为空字符串，则返回值为
 ...
 * @method static part SOUNDEX() 从str返回一个soundex字符串。 两个具有几乎同样探测的字符串应该具有同样的 soundex 字符串。一个标准的soundex 字符串的长度为4个字符，然而SOUNDEX() 函数会返回一个人以长度的字符串。 可使用结果中的SUBSTRING() 来得到一个标准 soundex 字符串。在str中，会忽略所有未按照字母顺序排列的字符。
 *         所有不在A-Z范围之内的国际字母符号被视为元音字母
 * @method static part SPACE() 返回一个由N 间隔符号组成的字符串
 * @method static part SUBSTRING($pos, $len=null) 不带有len 参数的格式从字符串str返回一个子字符串，起始于位置 pos。带有len参数的格式从字符串str返回一个长度同len字符相同的子字符串，起始于位置 pos。 使用 FROM的格式为标准 SQL 语法。也可能对pos使用一个负值。假若这样，则子字符串的位置起始于字符串结尾的pos
 *         字符，而不是字符串的开头位置。在以下格式的函数中可以对pos 使用一个负值。
 ...
 * @method static part CONV($from_base,$to_base) 不同数基间转换数字。返回值为数字的N字符串表示，由from_base基转化为 to_base 基。如有任意一个参数为NULL，则返回值为 NULL。自变量 N 被理解为一个整数，但是可以被指定为一个整数或字符串。最小基数为 2 ，而最大基数则为 36。 If to_base 是一个负数，则 N
 *         被看作一个带符号数。否则， N 被看作无符号数。 CONV() 的运行精确度为 64比特
 ...
 * @method static part TIMESTAMPADD($int_expr,$datetime_expr) 将整型表达式int_expr 添加到日期或日期时间表达式 datetime_expr中。 int_expr 的单位被时间间隔参数给定，该参数必须是以下值的其中一个： FRAC_SECOND、SECOND、 MINUTE、 HOUR、 DAY、 WEEK、 MONTH、 QUARTER或
 *         YEAR
 ...
 * @method static part UNIX_TIMESTAMP() 若无参数调用，则返回一个Unix timestamp ('1970-01-01 00:00:00' GMT 之后的秒数) 作为无符号整数。若用date 来调用UNIX_TIMESTAMP()，它会将参数值以'1970-01-01 00:00:00' GMT后的秒数的形式返回。date 可以是一个DATE 字符串、一个
 *         DATETIME字符串、一个 TIMESTAMP或一个当地时间的YYMMDD 或YYYMMDD格式的数字
 ...
 * @method static part RANDOM_BYTES() This function returns a binary string of len random bytes generated using the random number generator of the SSL library. Permitted values of len range from 1 to 1024. For
 *         values outside that range, RANDOM_BYTES() generates a warning and returns NULL.
 ...
 * @method static part SHA2($hash_length) Calculates the SHA-2 family of hash functions (SHA-224, SHA-256, SHA-384, and SHA-512). The first argument is the cleartext string to be hashed. The second argument
 *         indicates the desired bit length of the result, which must have a value of 224, 256, 384, 512, or 0 (which is equivalent to 256). STATEMENT_DIGEST($statement) Given an SQL statement as a string,
 *         returns the statement digest hash value as a string in the connection character set, or NULL if the argument is NULL. STATEMENT_DIGEST_TEXT($statement) Given an SQL statement as a string, returns
 *         the normalized statement digest as a string in the connection character set, or NULL if the argument is NULL.
 ...
 * @method static part VALIDATE_PASSWORD_STRENGTH() Given an argument representing a cleartext password, this function returns an integer to indicate how strong the password is. The return value ranges from 0
 *         (weak) to 100 (strong).
 *
 ...
 * @method static part CURRENT_ROLE() Returns a utf8 string containing the current active roles for the current session, separated by commas, or NONE if there are none. The value reflects the setting of the
 *         sql_quote_show_create system variable.
 ...
 * @method static part ICU_VERSION() The version of the International Components for Unicode (ICU) library used to support regular expression operations (see Section 12.5.2, “Regular Expressions”). This
 ...
 * JSON_OBJECT(...$keyThenValue) JSON_OBJECT('id', 87, 'name', 'carrot') {"id": 87, "name": "carrot"} Evaluates a (possibly empty) list of key-value pairs and returns a JSON object containing those pairs. An
 * error occurs if any key name is NULL or the number of arguments is odd. JSON_QUOTE($string) JSON_QUOTE('null') "null" Quotes a string as a JSON value by wrapping it with double quote characters and escaping
 * interior quote and other characters, then returning the result as a utf8mb4 string. Returns NULL if the argument is NULL. JSON_CONTAINS($target, $candidate, $path=null) Indicates by returning 1 or 0 whether
 * a given candidate JSON document is contained within a target JSON document, or—if a path argument was supplied—whether the candidate is found at a specific path within the target. Returns NULL if any
 * argument is NULL, or if the path argument does not identify a section of the target document. JSON_CONTAINS_PATH($json_doc, $oneORAll, ...$paths) Returns 0 or 1 to indicate whether a JSON document contains
 * data at a given path or paths. Returns NULL if any argument is NULL. An error occurs if the json_doc argument is not a valid JSON document, any path argument is not a valid path expression, or one_or_all is
 * not 'one' or 'all'. JSON_EXTRACT($json_doc, ...$paths) JSON_EXTRACT('[10, 20, [30, 40]]', '$[1]', '$[0]') [20, 10] "$.id" Returns data from a JSON document, selected from the parts of the document matched
 * by the path arguments. Returns NULL if any argument is NULL or no paths locate a value in the document. JSON_KEYS($json_doc, $path) JSON_KEYS('{"a": 1, "b": {"c": 30}}') ["a", "b"] Returns the keys from the
 * top-level value of a JSON object as a JSON array, or, if a path argument is given, the top-level keys from the selected path. Returns NULL if any argument is NULL, the json_doc argument is not an object, or
 * path, if given, does not locate an object. JSON_SEARCH($json_doc, $oneORAll, $search_str, $escape_char, ...$paths) Returns the path to the given string within a JSON document. Returns NULL if any of the
 * json_doc, search_str, or path arguments are NULL; no path exists within the document; or search_str is not found. JSON_ARRAY_APPEND($json_doc, ...$pathThenValue) Appends values to the end of the indicated arrays within a JSON document and returns the result. Returns NULL if any argument is NULL. JSON_ARRAY_INSERT($json_doc, ...$pathThenValue) Updates a JSON document, inserting into an array within the document and returning the modified document. Returns NULL if any argument is NULL. JSON_INSERT($json_doc, ...$pathThenValue) Inserts data into a JSON document and returns the result. Returns NULL if any argument is NULL. JSON_MERGE_PATCH(...$json_docs) Performs an RFC 7396 compliant merge of two or more JSON documents and returns the merged result, without preserving members having duplicate keys. JSON_MERGE_PRESERVE(...$json_docs) Merges two or more JSON documents and returns the merged result. Returns NULL if any argument is NULL. JSON_REMOVE($json_doc, ...$paths) Removes data from a JSON document and returns the result. Returns NULL if any argument is NULL. JSON_REPLACE($json_doc, ...$pathThenValue) Replaces existing values in a JSON document and returns the result. Returns NULL if any argument is NULL. JSON_SET($json_doc, ...$pathThenValue) Inserts or updates data in a JSON document and returns the result. Returns NULL if any argument is NULL or path, if given, does not locate an object. JSON_UNQUOTE($json_val) Unquotes JSON value and returns the result as a utf8mb4 string. Returns NULL if the argument is NULL. JSON_DEPTH($json_doc) Returns the maximum depth of a JSON document. Returns NULL if the argument is NULL. JSON_LENGTH($json_doc) Returns the length of a JSON document, or, if a path argument is given, the length of the value within the document identified by the path. Returns NULL if any argument is NULL or the path argument does not identify a value in the document. JSON_TYPE($json_val) Returns a utf8mb4 string indicating the type of a JSON value. JSON_VALID($val) Returns 0 or 1 to indicate whether a value is valid JSON. Returns NULL if the argument is NULL.
 ...
 * JSON_STORAGE_FREE($json_val) For a JSON column value, this function shows how much storage space was freed in its binary representation after it was updated in place using JSON_SET(), JSON_REPLACE(), or
 * JSON_REMOVE(). JSON_STORAGE_SIZE($json_val) This function returns the number of bytes used to store the binary representation of a JSON document.
 *
 ...
 * @method static part ANY_VALUE() This function is useful for GROUP BY queries when the ONLY_FULL_GROUP_BY SQL mode is enabled, for cases when MySQL rejects a query that you know is valid for reasons that
 *         MySQL cannot determine. The function return value and type are the same as the return value and type of its argument, but the function result is not checked for the ONLY_FULL_GROUP_BY SQL mode.
 * @method static part BIN_TO_UUID($swap_flag) It converts a binary UUID to a string UUID and returns the result. The binary value should be a UUID as a VARBINARY(16) value. The return value is a utf8 string
 *         of five hexadecimal numbers separated by dashes.
 ...
 * @method static part INET6_NTOA() Given an IPv6 or IPv4 network address represented in numeric form as a binary string, returns the string representation of the address as a string in the connection
 *         character set. If the argument is not a valid address, INET6_NTOA() returns NULL.
 * @method static part IS_IPV4() Returns 1 if the argument is a valid IPv4 address specified as a string, 0 otherwise.
 * @method static part IS_IPV4_COMPAT() This function takes an IPv6 address represented in numeric form as a binary string, as returned by INET6_ATON(). It returns 1 if the argument is a valid IPv4-compatible
 *         IPv6 address, 0 otherwise. IPv4-compatible addresses have the form ::ipv4_address.
 * @method static part IS_IPV4_MAPPED() This function takes an IPv6 address represented in numeric form as a binary string, as returned by INET6_ATON(). It returns 1 if the argument is a valid IPv4-mapped IPv6
 *         address, 0 otherwise. IPv4-mapped addresses have the form ::ffff:ipv4_address.
 ...
 * @method static part MASTER_POS_WAIT($log_pos,$timeout=null) 该函数对于控制主从同步很有用处。它会持续封锁，直到从设备阅读和应用主机记录中所有补充资料到指定的位置。返回值是其为到达指定位置而必须等待的记录事件的数目。若从设备SQL线程没有被启动、从设备主机信息尚未初始化、参数不正确或出现任何错误，则该函数返回
 *         NULL。若超时时间被超过，则返回-1。若在MASTER_POS_WAIT() 等待期间，从设备SQL线程中止，则该函数返回 NULL。若从设备由指定位置通过，则函数会立即返回结果
 ...
 * @method static part VALUES() 在一个INSERT … ON DUPLICATE KEY UPDATE …语句中，你可以在UPDATE 子句中使用 VALUES(col_name)函数，用来访问来自该语句的INSERT 部分的列值。换言之，UPDATE 子句中的 VALUES(col_name) 访问需要被插入的col_name
